...
import type { DBConnection as DatabaseConnection } from "@/main/utils";
...
export function NewConnectionDialog({
  ...
  const applyParsedToForm = (
    parsed: NonNullable<ReturnType<typeof parseConnectionString>>,
  ) => {
    form.setFieldValue("type", parsed.type);
    form.setFieldValue("host", parsed.host);
    form.setFieldValue(
      "port",
      parsed.port ?? (parsed.type === "postgres" ? 5432 : 3306),
    );

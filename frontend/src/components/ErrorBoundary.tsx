...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidUpdate(prevProps: ErrorBoundaryProps) {
    ...
    if (resetKeys && prevProps.resetKeys) {
      const changed = resetKeys.length !== prevProps.resetKeys.length ||
  ...
  render() {
    ...
    if (error) {
      if (typeof fallback === "function") {
        return (fallback as (error: Error, reset: () => void) => React.ReactNode)(error, this.reset);
      }
      ...
      return (
        <div className="w-full h-full flex items-center justify-center p-4">
          <div className="max-w-xl w-full border border-destructive/40 bg-destructive/10 text-destructive rounded p-4 shadow-sm">
            <div className="font-semibold mb-1">Something went wrong in this tab.</div>

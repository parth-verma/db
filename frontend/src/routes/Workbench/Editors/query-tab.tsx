...
export function QueryTab({ tabId }) {
  ...
  const runQueryMutation = useMutation({
    mutationFn: async ({
      ...
      const [cols, data] = await DBConnectionService.RunQuery(
        connectionId,
        query,
      );
    ...
    <ResizablePanelGroup direction="vertical">
      ...
      >
        <div className={"h-full flex flex-col max-h-full overflow-hidden"}>
          ...
          <div className="h-5 w-full bg-background/40 border-t border-border text-xs text-muted-foreground flex items-center px-2 justify-between">
            <div>
              {typeof queryResult?.rowCount === "function"
